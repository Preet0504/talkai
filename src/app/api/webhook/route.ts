...
export async function POST(req: NextRequest) {
  ...
  if (eventType === "call.session_started") {
    ...
    try {
      ...
      console.log(`Agent ${existingAgent.id} joined call ${meetingId}`);
    } catch (error) {
      console.error("Failed to connect agent:", error);
      return NextResponse.json(
        { error: "Failed to connect agent to call", details: String(error) },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      console.error("Failed to queue meeting processing:", error);
      // Don't fail the webhook if Inngest is unavailable
    }
